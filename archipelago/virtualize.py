...
import os
import pythunder
from .util import get_total_clb, get_group_size, get_max_num_col
...
def pnr_virtualize(interconnect, inputs, cwd, id_to_name, max_group):
    ...
    total_clb = get_total_clb(interconnect, max_group)
    partition_result = partition(inputs, id_to_name, total_clb, cwd)
    ...
    for cluster_id, (packed_file, io_placement) in partition_result.items():
        ...
        result[cluster_id] = cluster_result
    return result
